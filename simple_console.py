...
class SimpleConsole:
    ...
    def __init__(self, *, sda: int, scl: int, i2c_id: int = 0, freq: int = 400_000, addr: int | None = None, wrap: bool = True):
        """
    ...
    def _render(self) -> None:
        ...
        for r in range(self.rows):
            t = self._pad_right(self._to_str(visible[r])[: self.cols], self.cols)
            self.lcd.set_cursor(0, r)
            self.lcd.write(t)
        self.lcd.flush()
